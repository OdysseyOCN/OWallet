...
import com.ocoin.data.TransactionDisplay;
...
public class FragmentTransactionsAll extends FragmentTransactionsAbstract {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        WalletMainActivity ac = (WalletMainActivity) this.ac;
        ...
        if (storedwallets.size() == 0) {
            ...
            for (int i = 0; i < storedwallets.size(); i++) {
                try {
                        ...
                        public void onFailure(Call call, IOException e) {
                            if (isAdded()) {
                                ac.runOnUiThread(new Runnable() {
                                    ...
                                    public void run() {
                                        onItemsLoadComplete();
                                        ((WalletMainActivity) ac).snackError("No internet connection");
                                    }
                                });
                            }
                        }
                    ...
                    if (isAdded()) {
                        if (ac != null)
                            ((WalletMainActivity) ac).snackError("Can't fetch account balances. No connection?");
